...
Flexion과 abduction은 상완 벡터 `H`의 3D 좌표 성분을 몸통 LCS 기준 평면에 직접 투영해 계산한다. 즉 전방 성분은 flexion/extension, 측방 성분은 abduction/adduction으로 해석한다.
...
### 4.2 Flexion: YZ 평면 투영각

Flexion은 상완 벡터를 시상면인 `YZ` 평면에 투영한 뒤, 휴식 방향 `-Y`에서 전방 `+Z`로 얼마나 기울었는지 계산한다.
...
| `+90 deg` | 팔이 전방 `+Z`를 향함 |
| `-90 deg` | 팔이 후방 `-Z`를 향함 |
| `180 deg` | 팔이 위쪽 `+Y`를 향함 |

`H_z`가 거의 0이면 전후 성분이 없다고 보고 flexion을 `0 deg`로 둔다. 이 처리는 순수 abduction이나 머리 위 정점처럼 전방/후방 성분이 사라지는 자세에서 불필요한 180도 반환을 막기 위한 것이다.

### 4.3 Abduction: XY 평면 투영각

Abduction은 상완 벡터를 관상면인 `XY` 평면에 투영한 뒤, 휴식 방향 `-Y`에서 외측 방향으로 얼마나 기울었는지 계산한다.

```text
lateral = lat_sign * H_x
abduction = atan2(lateral, -H_y)
```
...
`lateral`이 거의 0이면 측방 성분이 없다고 보고 abduction을 `0 deg`로 둔다. 이 처리는 순수 flexion이나 머리 위 정점처럼 측방 성분이 사라지는 자세에서 불필요한 180도 반환을 막기 위한 것이다.

### 4.4 이 방식의 성질

- 순수 flexion에서는 `H_x = 0`이므로 abduction이 0이 된다.
- 순수 abduction에서는 `H_z = 0`이므로 flexion이 0이 된다.
- 두 값은 같은 총 거상각을 보존 분해한 값이 아니라, 서로 다른 평면에 투영한 독립 각도다.
- 팔이 정확히 머리 위 `+Y`에 가까워질수록 어느 평면으로 올라갔는지 현재 좌표 하나만으로는 모호하다. 현재 구현은 전방/측방 성분이 거의 0이면 해당 각도를 0으로 둔다.

예시는 다음과 같다.

| 자세 | 오른팔 기준 `H` | flexion | abduction |
|---|---|---:|---:|
| 휴식 | `(0, -1, 0)` | `0 deg` | `0 deg` |
| 순수 오른팔 abduction 90도 | `(1, 0, 0)` | `0 deg` | `90 deg` |
| 순수 flexion 90도 | `(0, 0, 1)` | `90 deg` | `0 deg` |
| 순수 flexion 150도 | `(0, 0.866, 0.5)` | `150 deg` | `0 deg` |
| scaption 90도 | `(0.707, 0, 0.707)` | `90 deg` | `90 deg` |
...
현재 방식은 몸통 기준 LCS에서 팔 벡터를 해석하므로, 카메라 좌표의 전역 회전이나 피험자의 몸통 기울기 영향을 줄인다. 또한 flexion과 abduction은 상완 벡터의 전방/측방 좌표 성분을 직접 반영하므로, 어떤 3D 방향 성분이 각 값에 기여하는지 해석하기 쉽다.
